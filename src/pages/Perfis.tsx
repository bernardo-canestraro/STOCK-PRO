...
export default function Perfis() {
  ...
  const fetchPerfis = () => {
    ...
    const url = `http://localhost:3001/perfis${q.toString() ? "?" + q.toString() : ""}`;
...
//   };
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    if (editingId) {
      fetch(`http://localhost:3001/perfis/${editingId}`, {
      method: "PUT",
    ...
    .then((updated: Perfil) => {
      setPerfis((prev) => prev.map((p) => (p.IdPerfil === editingId ? updated : p)));
      setShowForm(false);
      setEditingId(null);
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="space-y-2">
              <Label htmlFor="busca">Nome do perfil</Label>
            ...
            <div className="flex items-end gap-2">
              ...
              <Button variant="outline" onClick={() => { setBusca(""); setStatusFiltro("todos"); setPerfis([]); }}>
                Limpar
              </Button>
            </div>
          </div>
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {perfis.map((p) => (
                <TableRow key={p.IdPerfil}>
                  <TableCell className="font-medium">{p.IdPerfil}</TableCell>
                  <TableCell>{p.NomePerfil}</TableCell>
                  <TableCell>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-6">
          ...
          <div className="relative w-full max-w-md">
            <Card>
              <CardHeader>
                <CardTitle>{editingId ? "Editar Perfil" : "Novo Perfil"}</CardTitle>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="grid gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="nomePerfil">Nome do Perfil</Label>
                    <Input id="nomePerfil" value={form.NomePerfil} onChange={(e) => setForm({ ...form, NomePerfil: e.target.value })} placeholder="Ex: Administrador" />
                  </div>
